...
public interface MessageRepository extends Neo4jRepository<Message, String> {
	...
	@Query("MATCH (m : Message) WHERE ((m.from = $0 AND m.to = $1 AND m.readList[0] = 'true') OR " +
			...
			"RETURN m ORDER BY m.year, m.month, m.date, m.hours, m.minutes, m.seconds, m.milliseconds")
	...
	@Query("CREATE (message : Message{from: $0, to: $1, message: $2, groupName: $3, " +
			...
			"minutes: $9, seconds: $10, milliseconds: $11})")
	...
	@Query("MATCH (m : Message) WHERE ((m.from = $0 AND m.to = $1) OR (m.from = $1 AND m.to = $0) OR m.groupName = $1) " +
			"RETURN m ORDER BY m.year DESC, m.month DESC, m.date DESC, m.hours DESC, m.minutes DESC, m.seconds DESC, m.milliseconds DESC LIMIT 1")
	public Message getLatestMessageFromChat(String user, String chat);
}
